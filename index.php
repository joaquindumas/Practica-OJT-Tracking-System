...
<head>
  ...
  <title><?= e(APP_NAME) ?> — <?= $mode === 'register' ? 'Create Account' : 'Sign In' ?></title>
  <link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,600;9..144,700&family=DM+Sans:wght@300;400;500;600;700&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet" />
...
<div class="auth-wrap">
  <div class="auth-card">
    ...
    <?php if ($mode === 'login'): ?>
      <div class="auth-welcome">Sign in</div>
      ...
      <form method="POST" action="index.php">
        ...
        <div class="auth-field">
          ...
          <input class="auth-input" type="text" name="username" placeholder="Username" required value="<?= e($_POST['username'] ?? '') ?>" />
        </div>
        <div class="auth-field">
          <div class="auth-field-icon">
        ...
        <div class="auth-forgot"><a href="forgot_password.php">Forgot password?</a></div>
        <button type="submit" class="auth-btn">Sign in →</button>
      </form>
      <div class="auth-toggle">No account yet? <a href="index.php?mode=register">Create one</a></div>
    ...
    <?php elseif ($mode === 'register' && $reg_step === 1): ?>
      ...
      <form method="POST" action="index.php">
        ...
        <div class="auth-field">
          <div class="auth-field-icon"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z"/></svg></div>
          <input class="auth-input" type="text" name="name" placeholder="Full name" required value="<?= e($reg_data['name']) ?>" />
        </div>
        <div class="auth-field">
          <div class="auth-field-icon"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z"/></svg></div>
          <input class="auth-input" type="text" name="username" placeholder="Username" required value="<?= e($reg_data['username']) ?>" />
        </div>
        <div class="auth-field">
          <div class="auth-field-icon"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg></div>
          <input class="auth-input" type="number" name="required_hours" placeholder="Required OJT hours (e.g. 500)" min="1" step="0.5" value="<?= e($reg_data['required_hours']) ?>" required />
        </div>
        <div class="auth-field">
          <div class="auth-field-icon"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1s3.1 1.39 3.1 3.1v2z"/></svg></div>
          <input class="auth-input auth-input--pw" type="password" id="reg-pw" name="password" placeholder="Password (min. 6 characters)" required />
      ...
      <div class="auth-toggle">Already have an account? <a href="index.php?mode=login">Sign in</a></div>
    ...
    <?php elseif ($mode === 'register' && $reg_step === 2): ?>
      ...
      <div class="auth-sub">Step 2 of 2 — For password recovery.</div>
      ...
      <form method="POST" action="index.php">
        ...
        <div class="auth-group">
          ...
          <select class="auth-input" name="security_question" style="padding-left:14px;" required>
